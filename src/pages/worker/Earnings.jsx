    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="flow-card glow-orange-hover p-5 flex flex-col justify-between">
          ...
          <div className={`text-xs mt-2 font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('totalServiceValue', 'Total Service Value')}</div>
        </div>
        ...
        <div className="flow-card glow-orange-hover p-5 flex flex-col justify-between">
          ...
          <div className={`text-xs mt-2 font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t('tenPerJobWelfareCorpus', '₹10/job welfare corpus')}</div>
        </div>
        ...
        <div className="flow-card glow-emerald-hover p-5 flex flex-col justify-between">
          ...
          <div className={`text-xs mt-2 font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>{t('settledToBankUpi', 'Settled to your Bank/UPI')}</div>
        </div>
      </div>
        ...
        <div className={`flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b pb-3.5 ${isDark ? 'border-white/[0.06]' : 'border-slate-200'}`}>
          <div className="flex items-center gap-2.5">
            ...
            <h2 className={`text-base font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {t('perJobDissectionTitle', 'Per-Job Dissection & Payout Audit')}
            </h2>
          </div>
          <span className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            {t('autoGeneratedUponCompletion', 'Auto-generated upon digital completion')}
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full text-left text-xs ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
            <thead className={`border-b font-bold uppercase tracking-wider text-[11px] ${
              ...
              <tr>
                <th className="px-4 py-3.5">{t('jobIdHeader', 'Job ID')}</th>
                <th className="px-4 py-3.5">{t('completionDateHeader', 'Completion Date')}</th>
                <th className="px-4 py-3.5">{t('grossAmountHeader', 'Gross Amount')}</th>
                <th className="px-4 py-3.5">{t('coopFee5Header', 'Co-op Fee (5%)')}</th>
                <th className="px-4 py-3.5">{t('welfareFundHeader', 'Welfare Fund')}</th>
                <th className={`px-4 py-3.5 ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>{t('netWorkerPayoutHeader', 'Net Worker Payout')}</th>
                <th className="px-4 py-3.5">{t('invoiceActionHeader', 'Invoice Action')}</th>
              </tr>
            </thead>
